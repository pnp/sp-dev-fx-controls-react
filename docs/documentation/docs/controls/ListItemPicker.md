...
This control allows you to select one or more items from a list. The List can be filter to allow select items from a subset of items The item selection is based from a column value. The control will suggest items based on the inserted value.
...
| filterList | string | no | condition to filter list Item, same as $filter ODATA parameter|
