...
export const AddComment: React.FunctionComponent<IAddCommentProps> = (
    ...
    (
      ...
      if (newValue) {
        ...
        for (let index = 0; index < mentions.length; index++) {
          ...
          _addCommentText.current.text = _addCommentText.current.text.replace(
            mention.display,
            `@mention{${index}}`
          );
  ...
  const _addComment = useCallback(() => {
    setlistItemCommentsState({
      ...
      payload: ECommentAction.ADD,
    });
    setlistItemCommentsState({
      type: EListItemCommentsStateTypes.SET_ADD_COMMENT,
      payload: _addCommentText.current,
    });
    setSingleLine(true);
    setCommentText('');
  }, []);
  ...
  return (
    <>
      ...
      <div
        ...
        style={{ height: singleLine ? 35 : 'unset' }}
      >
        <MentionsInput
          value={commentText}
          onChange={_onChange}
