...
describe('<AccessibleChartTable />', () => {
  ...
  it('Should render only one table', (done) => {
    tableControl = mount(
      <AccessibleChartTable chartType={ChartType.Bar} data={dummyData} />
    );
    expect(tableControl.find(`div.${styles.accessibleTable}`)).to.have.length(
      1
    );
    done();
  });
  ...
  it('Should render a caption if one is provided', (done) => {
    ...
    expect(
      tableControl.find(`div.${styles.accessibleTable} table caption`).text()
    ).to.be.equal(dummyCaption);
    done();
  });
  ...
  it('Should render the same number of rows as there are data elements', (done) => {
    ...
    expect(
      tableControl.find(`div.${styles.accessibleTable} table tbody tr`)
    ).to.have.length(4);
    done();
  });
  ...
  it('Should render a table matching the data provided', (done) => {
    ...
    expect(
      tableControl
        .find(`div.${styles.accessibleTable} table tbody tr`)
        .at(0)
        .find('th')
        .at(0)
        .text()
  ...
  it('Should include a summary in the caption if one is provided', (done) => {
    ...
    expect(
      tableControl.find(`div.${styles.accessibleTable} table caption br`)
    ).to.have.length(1);
    expect(
      tableControl.find(`div.${styles.accessibleTable} table caption span`)
    ).to.have.length(1);
    expect(
      tableControl
        .find(`div.${styles.accessibleTable} table caption span`)
        .text()
    ).to.be.equals(dummySummary);
    done();
  });
  ...
  it('Should do nothing if there are no data labels', (done) => {
    tableControl = mount(
      <AccessibleChartTable
        chartType={ChartType.Bar}
        data={dummyDataNoLabels}
      />
    );
    expect(
      tableControl.find(`div.${styles.accessibleTable} table`)
    ).to.have.length(0);
    done();
  });
  ...
  it('Should render an X and Y label if axis labels are provided', (done) => {
    ...
    expect(
      tableControl
        .find(`div.${styles.accessibleTable} table thead tr`)
        .at(0)
        .find('th')
        .at(0)
        .text()
  ...
  it('Should render multi dataset labels', (done) => {
    ...
    expect(
      tableControl
        ...
        .at(2)
        .text()
    ).to.be.equal(dummyDatasetLabel2);
    done();
  });
