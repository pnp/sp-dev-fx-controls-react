...
export class SPHelper {
  ...
  public static getStoredFieldName(columnName: string): string {
    ...
    return Constants.FieldNamesMapping[columnName]
      ? Constants.FieldNamesMapping[columnName].storedName
      : columnName;
  }
  ...
  public static getFieldText(
    ...
    return new Promise<string>((resolve) => {
      const fieldCustomizerContext: FieldCustomizerContext =
        context as FieldCustomizerContext;
      const field: SPField = fieldCustomizerContext.field;
      ...
      switch (fieldType) {
        case "Note":
          ...
          break;
        case "DateTime":
          friendlyDisplay = SPHelper.getRowItemValueByName(
            listItem,
            `${fieldName}.FriendlyDisplay`
          );
          resolve(
            friendlyDisplay
              ? GeneralHelper.getRelativeDateTimeString(friendlyDisplay)
              : strFieldValue
          );
          break;
        case "User":
        ...
        case "URL":
          SPHelper.getFieldProperty(
            ...
            .then((format) => {
              ...
              resolve(
                SPHelper.getRowItemValueByName(listItem, `${fieldName}.desc`)
              );
            })
            .catch(() => {
              /* no-op; */
        ...
        case "TaxonomyFieldTypeMulti":
          terms = Array.isArray(fieldValue)
            ? <ITerm[]>fieldValue
            : <ITerm[]>[fieldValue];
        ...
        case "Computed":
          ...
          if (storedName === "URL") {
            resolve(
              this.getRowItemValueByName(listItem, "URL.desc") || strFieldValue
            );
          }
   ...
   */
  public static getFieldProperty(
    fieldId: string,
    propertyName: string,
    context: BaseComponentContext,
    fromSchemaXml: boolean
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ): Promise<any> {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    return new Promise<any>((resolve) => {
      let loadedViewFields: { [viewId: string]: IFields } =
        SPHelper._getLoadedViewFieldsFromStorage();
      ...
      if (fromSchemaXml) {
        SPHelper.getFieldSchemaXmlById(
          fieldId,
          context.pageContext.list.title,
          context
        ).then(
          (schemaXml) => {
        ...
        sp.web.lists
          .getByTitle(context.pageContext.list.title)
   ...
   * @param context SPFx Context
   */
  public static getLookupFieldListDispFormUrl(
    fieldId: string,
    context: BaseComponentContext
  ): Promise<string> {
    return new Promise<string>((resolve, reject) => {
      ...
      sp.web.lists
        .getByTitle(context.pageContext.list.title)
        .fields.getById(fieldId)
        .select("LookupWebId", "LookupList")()
        .then((f: IFieldLookupInfo) => {
          sp.site
            .openWebById(f.LookupWebId)
            .then((openedWeb) => {
              openedWeb.web
                .select("Url")()
                .then(
  ...
  public static getRowItemValueByName(
    listItem: ListItemAccessor,
    itemName: string
  ): any {
   ...
   * @param context Customizer's context
   */
  public static getFieldSchemaXmlById(
    fieldId: string,
    listTitle: string,
    context: BaseComponentContext
  ): Promise<string> {
   ...
   * @param context SPFx Context
   */
  public static getPageViewId(context: BaseComponentContext): string {
   ...
   * @param context SPFx context
   */
  public static async getUserProperties(
    loginName: string,
    context: BaseComponentContext
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ): Promise<any> {
  ...
  private static _updateSessionStorageLoadedViewFields(loadedViewFields: {
    [viewId: string]: IFields;
  }): void {
    try {
      if (window.sessionStorage) {
        window.sessionStorage.setItem(
          Constants.LoadedViewFieldsKey,
          JSON.stringify(loadedViewFields)
        );
      }
    } catch (error) {
  ...
  private static _getLoadedViewFieldsFromStorage(): {
    ...
    try {
      if (window.sessionStorage) {
        const loadedViewFields = sessionStorage.getItem(
          Constants.LoadedViewFieldsKey
        );
        if (loadedViewFields) {
