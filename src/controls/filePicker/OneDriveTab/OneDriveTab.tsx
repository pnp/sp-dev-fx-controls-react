...
import { IOneDriveTabProps, IOneDriveTabState, IOneDriveFile, ViewType } from '.';
...
import { IDimensions, IGetListDataAsStreamResult, IRow, IParentFolderInfo } from '../../../services/IOneDriveService';
...
export default class OneDriveTab extends React.Component<IOneDriveTabProps, IOneDriveTabState> {
  ...
  private _oneDriveService: OneDriveService = undefined;
    ...
    const columns: IColumn[] = [
      ...
      },
      {
        ...
        onRender: (item: IOneDriveFile) => {
          ...
          return <span>{item.fileSizeDisplay ? GeneralHelper.formatBytes(item.fileSizeDisplay, 1) : undefined}</span>;
        }
      },
      {
        key: 'colShared',
  ...
  */
  ...
  private _buildOneDriveAbsoluteUrl = (root: string, relativeUrl: string) => {
    const siteUrl: string = GeneralHelper.getAbsoluteDomainUrl(root);
