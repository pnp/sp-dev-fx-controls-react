...
import {
	IRichTextCustomFormattingStyles,
	IRichTextProps,
	IRichTextState,
} from './RichText.types';
...
const CUSTOM_FORMATTING_STYLE_SELECTORS = {
	normal: '',
	header2: 'h2',
	header3: 'h3',
	header4: 'h4',
	...
	Record<keyof IRichTextCustomFormattingStyles, string>
>;

const UNITLESS_CSS_PROPERTIES: ReadonlySet<string> = (() => {
	...
	const baseProperties = Array.from(properties);

	baseProperties.forEach((propertyName) => {
		const capitalizedPropertyName =
			propertyName.charAt(0).toUpperCase() + propertyName.slice(1);

		prefixes.forEach((prefix) => {
			properties.add(`${prefix}${capitalizedPropertyName}`);
		});
	});
...
export class RichText extends React.Component<IRichTextProps, IRichTextState> {
	...
	private _customFormattingStylesElement: HTMLStyleElement = undefined;
	...
	public componentDidMount(): void {
		...
		this.applyCustomFormattingStyles();
	}
	...
	public componentWillUnmount(): void {
		...
		this.removeCustomFormattingStylesElement();
	}
	...
	public componentDidUpdate(
		...
		const enteredEditMode = this.props.isEditMode && !prevProps.isEditMode;
		...
		if (
			prevProps.customStyles !== this.props.customStyles ||
	...
	public render(): React.ReactElement<IRichTextProps> {
		...
		if (!isEditMode) {
			return (
				<div className={css(CONTAINER_CLASS)} style={themeStyle}>
					{renderLabel}
					<div
						id={this._richTextId}
						className={css(
	...
	private handleClickOutside = (event: Event): void => {
		const outside: boolean = !elementContains(
			this._wrapperRef,
	...
	/**
	 * Build and inject object-based custom formatting styles scoped to this editor instance.
	 */
	private applyCustomFormattingStyles = (): void => {
		const formattingStyles = this.props.customStyles;
		// No custom formatting means any previously injected rules should be removed.
		if (!formattingStyles) {
			this.removeCustomFormattingStylesElement();
			return;
		}
		...
		(
			Object.keys(CUSTOM_FORMATTING_STYLE_SELECTORS) as Array<
				keyof IRichTextCustomFormattingStyles
			>
		).forEach((key) => {
			const styleObject = formattingStyles[key];
			const declaration = this.serializeStyleObject(styleObject);
			// Skip empty style blocks so we don't emit empty CSS rules.
			if (!declaration) {
				return;
			}

			const selectorSuffix = CUSTOM_FORMATTING_STYLE_SELECTORS[key];
			// Build selectors per key, for example: root h2, root blockquote, root li.
			const selector = selectorSuffix
				? baseSelectors.map((base) => `${base} ${selectorSuffix}`).join(', ')
				: baseSelectors.join(', ');
			rules.push(`${selector} { ${declaration} }`);
		});

		// Store one style sheet text blob for this instance.
		...
		// Create the style element once, then only update its text content.
		if (!this._customFormattingStylesElement) {
			this._customFormattingStylesElement = document.createElement('style');
			this._customFormattingStylesElement.setAttribute(
				'data-richtext-formatting',
				this._richTextId
			);
			document.head.appendChild(this._customFormattingStylesElement);
		}

		this._customFormattingStylesElement.textContent = styleText;
	};

	private removeCustomFormattingStylesElement = (): void => {
		// Guard against repeated cleanup calls.
		if (this._customFormattingStylesElement?.parentNode) {
			this._customFormattingStylesElement.parentNode.removeChild(
				this._customFormattingStylesElement
			);
		}

		this._customFormattingStylesElement = undefined;
	};

	private serializeStyleObject = (style?: React.CSSProperties): string => {
		if (!style) {
			return '';
		}

		// Convert a React style object into plain CSS declarations.
		const declarations = Object.entries(style)
			.map(([propertyName, value]) => {
				...
				// Keep CSS variables intact; convert camelCase names to kebab-case.
				...
				const cssValue =
					typeof value === 'number'
						? this.convertNumericStyleValue(propertyName, value)
						: String(value);
	...
	private convertNumericStyleValue = (
		_propertyName: string,
		value: number
	): string => {
		// Keep custom unitless properties untouched; default all others to px.
		if (value === 0 || UNITLESS_CSS_PROPERTIES.has(_propertyName)) {
			return String(value);
		}
	...
	/**
	 * Resolve theme tokens at runtime to support pre-compiled package CSS.
	 */
	private getThemeStyle = (): React.CSSProperties => {
		return {
			'--rt-neutralLighterAlt': ThemeColorHelper.GetThemeColor(
				styles.NeutralLighterAlt
			),
			'--rt-neutralPrimary': ThemeColorHelper.GetThemeColor(
				styles.NeutralPrimary
			),
			'--rt-neutralTertiary': ThemeColorHelper.GetThemeColor(
				styles.NeutralTertiary
			),
			'--rt-themePrimary': ThemeColorHelper.GetThemeColor(styles.ThemePrimary),
			'--rt-neutralLighter': ThemeColorHelper.GetThemeColor(
				styles.NeutralLighter
			),
			'--rt-neutralDark': ThemeColorHelper.GetThemeColor(styles.NeutralDark),
			'--rt-themeDarker': ThemeColorHelper.GetThemeColor(styles.ThemeDarker),
			'--rt-white': ThemeColorHelper.GetThemeColor(styles.White),
