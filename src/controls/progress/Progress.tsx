...
export class Progress extends React.Component<IProgressProps, IProgressState> {
    ...
    public componentDidUpdate() {
        ...
        //
        if (this.props.longRunningText && this.props.longRunningTextDisplayDelay &&
    ...
    public render(): React.ReactElement<IProgressProps> {
        ...
        if (showIndeterminateOverallProgress) {
            ...
            progressSubTtext = actionIndex !== undefined ? `${(progress * 100).toFixed(0)}%` : '';
        }
        ...
        //
        return <div className={css(styles.progress, className)} style={{ height: height || 'auto' }}>
            {
                ...
                <div className={css(styles.header, headerClassName)}>
                    ...
                    {showOverallProgress && progressSubTtext &&
                        <span className={styles.progressSubtext}>{progressSubTtext}</span>}
                    {this.state.showLongRunningText &&
                        <div className={styles.longRunningText}>{longRunningText}</div>}
                </div>
            }
