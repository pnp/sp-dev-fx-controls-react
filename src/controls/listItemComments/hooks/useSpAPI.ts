...
export const useSpAPI = (): returnObject => {
  ...
  //https://spteck.sharepoint.com/sites/ThePerspective/_api/web/lists(@a1)/GetItemById(@a2)/Comments(@a3)?@a1=%27%7BE738C4B3%2D6CFF%2D493A%2DA8DA%2DDBBF4732E3BF%7D%27&@a2=%2729%27&@a3=%273%27
  ...
  const deleteComment = useCallback(
    async (commentId: number): Promise<void> => {
      ...
      const _endPointUrl = `${webUrl}/_api/web/lists(@a1)/GetItemById(@a2)/Comments(@a3)?@a1='${listId}'&@a2='${itemId}'&@a3='${commentId}'`;
  ...
  const addComment = useCallback(
    async (comment: IAddCommentPayload): Promise<IComment> => {
      const spHttpClient = serviceScope.consume(SPHttpClient.serviceKey);
      if (!spHttpClient) return;
      const _endPointUrl = `${webUrl}/_api/web/lists(@a1)/GetItemById(@a2)/Comments()?@a1='${listId}'&@a2='${itemId}'`;
      const spOpts: ISPHttpClientOptions = {
